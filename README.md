...
These endpoints handle the patient workflow from a specific doctor's page.
...
The `doctorId` does not need to be sent in the request body because the doctor is identified by the URL.
...
This operation removes the doctor-patient relationship rather than permanently deleting the patient.
...
After removing from Doctor A:

Patient
└── doctorId = null
```

This allows the patient record to remain available in the general patient management system.
